...
class Fuse extends EventEmitter {
    ...
    _wrapOperations(ops) {
        ...
        if (ops.read) {
            wrapped.read = (path, fd, buffer, length, offset, cb) => {
                try {
                    ops.read(path, fd, buffer, length, offset, (firstArg, secondArg) => {
                        // Handle both patterns:
                        // 1. (err, bytesRead) - error first
                        // 2. (bytesRead, buffer) - bytes read first (for compatibility)
                        if (typeof firstArg === 'number' && firstArg >= 0 && secondArg === buffer) {
                            // Pattern 2: (bytesRead, buffer)
                            cb(firstArg, buffer);
                        } else if (firstArg) {
                            // Pattern 1: error
                            cb(errnoToCode(firstArg.errno || firstArg));
                        } else {
                            // Pattern 1: (null, bytesRead)
                            cb(secondArg || 0, buffer);
                        }
                    });
                } catch (e) {
                    cb(errnoToCode(e.errno || exports.EIO));
    ...
    static unmount(mountPath, callback) {
        ...
        exec(`fusermount3 -u ${mountPath}`, (err) => {
            callback(err);
        });
    }
...
// Add error constants to Fuse class
Fuse.EPERM = fuse3_napi.EPERM;
Fuse.ENOENT = fuse3_napi.ENOENT;
Fuse.EIO = fuse3_napi.EIO;
Fuse.EACCES = fuse3_napi.EACCES;
Fuse.EEXIST = fuse3_napi.EEXIST;
Fuse.ENOTDIR = fuse3_napi.ENOTDIR;
Fuse.EISDIR = fuse3_napi.EISDIR;
Fuse.EINVAL = fuse3_napi.EINVAL;
Fuse.ENOSPC = fuse3_napi.ENOSPC;
Fuse.EROFS = fuse3_napi.EROFS;
Fuse.EBUSY = fuse3_napi.EBUSY;
Fuse.ENOTEMPTY = fuse3_napi.ENOTEMPTY;
