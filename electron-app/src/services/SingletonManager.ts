...
export class SingletonManager {
  private options: Required<SingletonOptions>;
  private lockFilePath: string;
  private projfsLockPath: string;
   ...
   */
  async startIPCServers(messageHandler: (message: SingletonMessage) => void): Promise<void> {
    ...
    if (process.platform === 'win32') {
      try {
        this.pipeServer = net.createServer((socket) => {
          socket.on('data', (data) => {
            try {
              const message = JSON.parse(data.toString()) as SingletonMessage;
   ...
   */
  async sendToMainInstance(message: SingletonMessage): Promise<boolean> {
   ...
   */
  private cleanupProjFSLock(): void {
    try {
      if (fs.existsSync(this.projfsLockPath)) {
  ...
  private readProjFSLockFile(): any {
    try {
  ...
  private sendViaPipe(message: SingletonMessage): Promise<boolean> {
  ...
  private sendViaTCP(message: SingletonMessage): Promise<boolean> {
    return new Promise((resolve) => {
      const client = net.createConnection(
        { port: this.options.port, host: '127.0.0.1', timeout: 1000 },
        () => {
