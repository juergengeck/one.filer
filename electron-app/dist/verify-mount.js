...
export async function verifyProjFSMount(mountPath) {
    ...
    try {
        ...
        for (const dir of expectedDirs) {
            ...
            try {
                ...
                console.log(`[MountVerification] ${dir}: ERROR - ${err.message}`);
                issues.push(`${dir} cannot be accessed: ${err.message}`);
            }
        }
        if (issues.length > 0) {
        ...
        try {
            ...
            console.error(`[MountVerification] Could not list /debug contents: ${err.message}`);
        }
    }
    catch (err) {
        console.error(`[MountVerification] ERROR: Could not verify mount: ${err.message}`);
    }
}
//# sourceMappingURL=verify-mount.js.map