...
import {getIdObject, getVersionNodeByNodeHash, getVersionsNodeHashes} from '@refinio/one.core/lib/storage-versioned-objects.js';
...
async function addVersionedObjToAccessibleObjectsMap(
    ...
    for (const version of versions) {
        const node = await getVersionNodeByNodeHash(version);
        const accessibleObject = getOrCreate(accessibleObjects, node.obj.data, {
            type: 'versioned',
            hash: node.obj.data as SHA256Hash<OneVersionedObjectTypes>,
            idHash,
            oneType: idObj.$type$,
            timestamps: []
        });
        ...
        if (accessibleObject.type === 'versioned') {
            accessibleObject.timestamps.push(node.obj.creationTime);
        }
    }
}
