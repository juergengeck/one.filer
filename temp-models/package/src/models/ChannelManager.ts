...
export default class ChannelManager {
    ...
    public async createChannel(
        ...
        try {
            ...
            await storeVersionObjectAsChange(
                {
                    $type$: 'ChannelInfo',
                    id: channelId,
                    owner
        ...
        await serializeWithType(ChannelManager.registryLockName, async () => {
            ...
            await Promise.all(
                [...registry.channels].map(async channelIdHash => {
                    try {
                        const channelInfo = await getCurrentVersion(channelIdHash);
                        this.channelInfoCache.set(channelIdHash, channelInfo);
                    } catch (_e) {
    ...
    private async internalChannelPost(
        ...
        return await storeVersionObjectAsChange({
            $type$: 'ChannelInfo',
            id: channelId,
            owner: channelOwner,
            head: newHeadHash
