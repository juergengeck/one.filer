...
function parseInitialConfig(certificateRoleConfig: CertificateRoleConfig): CertificateRoleConfig {
    return {
        propagateRolePersonInstancesToOthers: false,
        propagateRolePersonCertificatesToOthers: false,
...
export default abstract class CertificateRoleModel extends Role {
    ...
    public async shareWith(shareWithPersonIds: SHA256IdHash<Person>[], options: {rolePersonId?: SHA256IdHash<Person>, forcePropagate?: boolean} = {}): Promise<void> {
        ...
        await this.sharePersons({
            ...
            shareWith: shareWithPersonIds,
            forcePropagate: options.forcePropagate
        });
    }
    ...
    protected async filterRolePersonIdsToShare(filterRolePersonIds: SHA256IdHash<Person>[] | undefined, forceFilteredRolePersonIdsPropagate: boolean = false): Promise<SHA256IdHash<Person>[]> {
        ...
        const rolePersonIds = this.certificateRoleConfig.propagateRolePersonInstancesToOthers ? allRolePersonIds : allRolePersonIds.includes(myIndentity) ? [myIndentity] : [];
     ...
     * @param options.forcePropagate - Whether to force the filtered role person ids to propagate. Default is false.
    */
    protected async sharePersons(options: {filterRolePersonIds?: SHA256IdHash<Person>[], shareWith?: SHA256IdHash<Person>[], forcePropagate?: boolean} = {}): Promise<void> {
        const rolePersonIds = await this.filterRolePersonIdsToShare(options.filterRolePersonIds, options.forcePropagate);
        if (rolePersonIds.length === 0) {
            // no role person/s to share
        ...
        for (const personId of rolePersonIds) {
            ...
            if (options.forcePropagate || this.certificateRoleConfig.propagateRolePersonCertificatesToOthers) {
