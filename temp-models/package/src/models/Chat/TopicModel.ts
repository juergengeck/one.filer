...
export default class TopicModel extends Model {
    ...
    public async createOneToOneTopic(
        ...
        const nameAndId = this.createOneToOneTopicId(from, to);
    ...
    /**
     * Create a one to one topic from a topic id.
     * @param topicId
     */
    public async createOneToOneTopicFromTopicId(topicId: string): Promise<Topic> {
        this.state.assertCurrentState('Initialised');

        const [from, to] = this.getOneToOneChatParticipants(topicId);
        return await this.createOneToOneTopic(from, to);
    }

    /**
     * Create a one to one topic id.
     * @param from person id
     * @param to person id
     * @returns topicId for the one to one chat
     */
    public createOneToOneTopicId(from: SHA256IdHash<Person>, to: SHA256IdHash<Person>): string {
