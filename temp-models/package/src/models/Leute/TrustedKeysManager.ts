...
import {getObjectByIdHash, getVersionNodeByNodeHash, getVersionsNodeHashes} from '@refinio/one.core/lib/storage-versioned-objects.js';
import {
    hexToUint8Array,
        ...
        for (const someone of [me, ...others]) {
            for (const identity of someone.identities()) {
                for (const profileModel of await someone.profiles(identity)) {
                    ...
                    const versionMapEntries = await getVersionsNodeHashes(profileIdHash);
                    if (versionMapEntries === undefined) {
                        continue;
                    }

                    for (const versionMapEntry of versionMapEntries) {
                        const node = await getVersionNodeByNodeHash(versionMapEntry);
                        const profileHash = node.obj.data;
                        await profileModel.loadVersion(profileHash);
                        const profileData = await this.getProfileData(
                            profileHash,
                            node.obj.creationTime
                        );
