...
export default class GroupModel extends Model {
        ...
        this.onUpdated.onListen(() => {
            if (this.onUpdated.listenerCount() === 0) {
                const d1 = objectEvents.onNewVersion(
                    async (result: VersionedObjectResult<Group>) => {
                ...
                const d2 = objectEvents.onNewVersion(
                    async (result: VersionedObjectResult<GroupProfile>) => {
                        if (result.idHash === this.profileIdHash) {
                            await this.onUpdated.emitAll();
                        }
                    },
    ...
    public static async constructWithNewGroup(groupName?: string): Promise<GroupModel> {
        ...
        try {
            ...
            groupResult = await storeVersionObjectAsChange(newGroup);
        }
        ...
        try {
            ...
            profileResult = await storeVersionObjectAsChange(newProfile);
        }
    ...
    public async saveAndLoad(): Promise<void> {
        ...
        const profileResult = await storeVersionObjectAsChange({
            $type$: 'GroupProfile',
        ...
        const groupResult = await storeVersionObjectAsChange({
            $type$: 'Group',
