...
import { getObjectByIdHash, storeVersionObjectAsChange } from '@refinio/one.core/lib/storage-versioned-objects.js';
...
async function getChannelRegistry() {
    const registryIdHash = await calculateIdHashOfObj({
...
describe('Channel Iterators test', () => {
    before(async () => {
        ...
        owner = (await storeVersionObjectAsChange({
            $type$: 'Person',
    ...
    it('should add data to created channels', async () => {
        await Promise.all(channelsIdentifiers.map(async (identifier) => {
    ...
    it('should add data to a NO OWNER channel and retrieve them', async () => {
        ...
        await channelManager.postToChannel(noOwnerChannelId, {
            $type$: 'BodyTemperature',
            temperature: Math.random()
        }, null);
    ...
    it('should create second tier of channels but for another user and add data to them', async () => {
        ...
        await Promise.all(channelsIdentifiers.map(async (identifier) => {
            for (let i = 0; i < howMany; i++) {
                await channelManager.postToChannel(identifier, { $type$: 'BodyTemperature', temperature: Math.random() }, owner);
            }
        }));
        await wait(100);
        for (const channelId of channelsIdentifiers) {
    ...
    it('should test getObjects with queryOptions.count, queryOptions.form and queryOption.to' +
        ' and OWNER', async () => {
        const count = 10;
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjects({ channelId, owner });
    ...
    it('should test getObjects with queryOptions.count, queryOptions.form and' +
        ' queryOption.to and NO-OWNER', async () => {
        const count = 10;
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjects({ channelId });
            const from = retrievedObjects[retrievedObjects.length / 2].creationTime;
    ...
    it('should test getObjectsWithType with specific type and queryOptions.from and' +
        ' queryOptions.to and OWNER', async () => {
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjectsWithType('BodyTemperature', {
                channelId,
                owner: owner
    ...
    it('should test getObjectsWithType with specific type and queryOptions.count,' +
        ' queryOptions.form and queryOption.to and OWNER', async () => {
        const count = 10;
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjectsWithType('BodyTemperature', {
                owner: owner,
            ...
            const to = retrievedObjects[retrievedObjects.length - 2].creationTime;
            const trueLen = retrievedObjects.length / 2 - 1;
            const objectsFromToWithOwner = await channelManager.getObjectsWithType('BodyTemperature', {
                owner: owner,
                channelId,
                count: count,
                from: from,
    ...
    it('should test getObjectsWithType with specific type and queryOptions.from and' +
        ' queryOptions.to and NO-OWNER', async () => {
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjectsWithType('BodyTemperature', {
                channelId
            });
    ...
    it('should test getObjectsWithType with specific type and queryOptions.count,' +
        ' queryOptions.form and queryOption.to and NO-OWNER', async () => {
        const count = 10;
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjectsWithType('BodyTemperature', {
                channelId
            ...
            const from = retrievedObjects[retrievedObjects.length / 2].creationTime;
            const to = retrievedObjects[retrievedObjects.length - 2].creationTime;
            const objectsFromToWithOwner = await channelManager.getObjectsWithType('BodyTemperature', { channelId, count: count, from: from, to: to });
            expect(objectsFromToWithOwner).to.have.length(count);
        }
    });
    it('should test getObjectsWithType with no specific type and NO-OWNER', async () => {
        for (const channelId of channelsIdentifiers) {
            const retrievedObjects = await channelManager.getObjectsWithType('Person', {
                channelId
            });
            expect(retrievedObjects).to.have.length(0);
        }
...
//# sourceMappingURL=ChannelIterators-test.js.map