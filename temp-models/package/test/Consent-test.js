...
describe('Consent', () => {
    ...
    it('should have the right firstConsentDate', async function () {
        ...
        const allConsents = await Promise.all(allChannelEntrys.map(entry => getObjectWithType(entry.data.data, 'Consent_1_1_0')));
        if (consentModel.firstConsentDate === undefined) {
            throw new Error('consentModel.firstConsentDate is undefined');
        }
        expect(consentModel.firstConsentDate.toISOString()).to.be.equal(allConsents[0].isoStringDate);
    });
