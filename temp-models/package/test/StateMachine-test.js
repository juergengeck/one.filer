...
function createStateMachineWithoutHistory(hasHistory) {
    const subSMLvl2 = new StateMachine();
    ...
    const subSMLvl1 = new StateMachine();
...
describe('StateMachine test', () => {
    ...
    it('Trigger invalid event ', async () => {
        ...
        let triggered = false;
        sm.onStateChange((_state, _newState, _event) => {
            triggered = true;
        });
        try {
    ...
    it('Trigger an invalid transition that it is not related to the current state', async (done) => {
        ...
        try {
            ...
            expect(error, error).to.be.instanceof(Error);
            expect(error.message).to.include('The transition does not exist from the current state with the specified event');
            done();
        }
    ...
    it('Check events are emitted for subStateMachines', async () => {
        ...
        subSMLvl1.onEnterState((enteredState) => {
            onEnterStateTriggeredLevel1++;
            expect(enteredState).to.be.equal('B');
        });
        subSMLvl1.onLeaveState((leftState) => {
        ...
        subSMLvl2.onEnterState((enteredState) => {
            onEnterStateTriggeredLevel2++;
            expect(enteredState).to.be.equal('B');
        });
        subSMLvl2.onLeaveState((leftState) => {
