...
describe('MultiUser Test', () => {
    async function waitForState(state, delay = 500) {
    ...
    describe('Register & Erase', () => {
        ...
        it('should test if logoutAndErase() throws an error when it is called twice', async () => {
            ...
            await chai
                ...
                .to.eventually.be.rejectedWith('The transition does not exist from the current state with the specified event');
        });
        ...
        it('should test if register(email, secret, instanceName) throws an error when user' +
            ' already exist', async () => {
            await chai
        ...
        it('should test if register can create multiple users and erase each one of them' +
            ' successfully', async () => {
            await multiUserWorkflow.register('test$email_3', 'test$secret_3', 'test$instanceName_3');
            await waitForState('logged_in');
            await multiUserWorkflow.logoutAndErase();
            await waitForState('logged_out');
            await multiUserWorkflow.register('test$email_4', 'test$secret_4', 'test$instanceName_4');
            await waitForState('logged_in');
            await multiUserWorkflow.logoutAndErase();
            await waitForState('logged_out');
        });
    ...
    describe('Login & Logout', () => {
        ...
        it('should test if login(email, secret, instanceName) throws an error when the user was not registered', async () => {
            await chai
                .expect(multiUserWorkflow.login('test$email_5', 'test$secret_5', 'test$instanceName_5'))
        ...
        it.skip('should test if login(email, secret, instanceName) throws an error when the user' +
            ' inputs the wrong secret', async () => {
            await chai
    ...
    describe('LoginOrRegister', () => {
        it('should test if loginOrregister(email, secret, instanceName) is successfuly when no user was registered', async () => {
            await multiUserWorkflow.loginOrRegister('test$email_6', 'test$secret_6', 'test$instanceName_6');
            await waitForState('logged_in');
            await multiUserWorkflow.logoutAndErase();
            await waitForState('logged_out');
        });
        ...
        it('should test if loginOrregister(email, secret, instanceName) throws an error when the user double loginOrRegister', async () => {
            ...
            await waitForState('logged_in');
            await chai
                .expect(multiUserWorkflow.loginOrRegister(user1.email, user1.secret, user1.instance))
                .to.eventually.be.rejectedWith('The transition does not exist from the current state with the specified event');
            await multiUserWorkflow.logout();
            await waitForState('logged_out');
        });
        it.skip('should test if loginOrregister(email, secret, instanceName) throws an error when the user was' +
            ' already registered and it calls the function with the wrong secret', async () => {
            await chai
                .expect(multiUserWorkflow.loginOrRegister(user1.email, 'wrong-secret', user1.instance))
                .to.eventually.be.rejectedWith('The provided secret is wrong');
        });
    });
    describe('isRegistered', () => {
        it('should test if isRegistered() returns true when the user is registered', async () => {
            expect(await multiUserWorkflow.isRegistered(user1.email, user1.instance)).to.be.equal(true);
        });
        it('should test if isRegistered() returns false when the user is not registered', async () => {
            expect(await multiUserWorkflow.isRegistered('test$email_5', 'test$instanceName_5')).to.be.equal(false);
        });
    });
});
//# sourceMappingURL=MultiUser-test.js.map