...
import { getVersionsHashes } from '@refinio/one.core/lib/storage-versioned-objects.js';
let channelManager;
let testModel;
...
describe('Channel Manager test', () => {
    ...
    it('should iterate differences in versions', async () => {
        const channelId = 'iterate differences';
        await channelManager.createChannel(channelId);
        await channelManager.postToChannel(channelId, { $type$: 'BodyTemperature', temperature: 1 });
        await channelManager.postToChannel(channelId, { $type$: 'BodyTemperature', temperature: 6 });
        ...
        await channelManager.internalChannelPost(channelId, channels[0].owner, { $type$: 'BodyTemperature', temperature: 9 }, undefined, firstValuesAsc[1].creationTime.getTime() - 1);
        const versionMap = (await getVersionsHashes(hash));
        const elements1 = [];
        for await (const entry of ChannelManager.differencesIteratorMostCurrent(versionMap[1], versionMap[versionMap.length - 1])) {
            elements1.push((await getObject(entry.dataHash)).temperature);
        }
        const elements2 = [];
        for await (const entry of ChannelManager.differencesIteratorMostCurrent(versionMap[2], versionMap[versionMap.length - 1])) {
