...
const EP_ERRORS: {[key: string]: {message: string;}} = {
    'EP-KEYMISSMATCH': {message: 'Key does not match your previous visit'},
    'EP-KEYFAILED': {message: 'Failed to load keys object for person {person}'},
    'EP-PERSONIDMISMATCH': {message: 'The person id does not match the one we have on record.'},
    'EP-KEYS-OWNER-MISMATCH': {message: 'Received keys object does not belong to the transmitted person id object'}
};
...
export async function verifyAndExchangePersonId(
    ...
    if (keys.remotePersonKeys.owner !== personIds.remotePersonId) {
        throw createError('EP-KEYS-OWNER-MISMATCH', {message: EP_ERRORS['EP-KEYS-OWNER-MISMATCH'].message});
    }
    ...
    if (matchRemotePersonId && personIds.remotePersonId !== matchRemotePersonId) {
        throw createError('EP-PERSONIDMISMATCH', {message: EP_ERRORS['EP-PERSONIDMISMATCH'].message});
    }
    ...
    if (keyComparisionResult === 'exception') {
        throw createError('EP-KEYFAILED', {message: EP_ERRORS['EP-KEYFAILED'].message.replace('{person}', personIds.remotePersonId)});
    }

    if (!isNewPerson && keyComparisionResult === 'nomatch' && !skipLocalKeyCompare) {
        throw createError('EP-KEYMISSMATCH', {message: EP_ERRORS['EP-KEYMISSMATCH'].message});
    }
