...
describe('Connection test', () => {
    ...
    afterEach('Shutdown Connections', async function () {
        ...
        await new Promise((resolve, reject) => {
            if (webSocketServer.webSocketServer) {
                webSocketServer.webSocketServer.close((err) => {
                    if (err) {
                        reject(err);
                    }
                    else {
    ...
    it('promise plugin', async function () {
        ...
        connClient.send('HELLO Server!');
        connServer.send('HELLO Client!');
        expect(await connClient.promisePlugin().waitForStringMessage()).to.be.equal('HELLO Client!');
        expect(await connServer.promisePlugin().waitForStringMessage()).to.be.equal('HELLO Server!');
    });
});
describe('Connection test - with promises', () => {
    ...
    it('tests waitForMessageWitType: no failures in two messages', async function () {
        ...
        expect(await connServer.promisePlugin().waitForJSONMessageWithType('mytype1')).to.be.eql(message1);
        const message2 = {
            type: 'mytype2',
            message: 'ABC'
        };
        connClient.send(JSON.stringify(message2));
        expect(await connServer.promisePlugin().waitForJSONMessageWithType('mytype2')).to.be.eql(message2);
    });
