...
import { PathMapper } from '../utils/PathMapper.js';
import { AttributeConverter } from '../utils/AttributeConverter.js';
 ...
 */
export class IFileSystemToProjFSAdapter implements ProjFSCallbacks {
    ...
    constructor(
        ...
        this.logger = new Logger('IFileSystemToProjFSAdapter', options?.logLevel);
        this.pathMapper = new PathMapper(virtualRoot);
        this.attributeConverter = new AttributeConverter();
        this.cache = new CacheManager(options?.cacheSize || 100 * 1024 * 1024); // 100MB default
    ...
    async onGetFileData(
        ...
        try {
            ...
            // Check content cache
            const cachedContent = await this.cache.getFileContent(
                virtualPath, 
                Number(byteOffset), 
                length
            );
            ...
            // Cache the content
            await this.cache.putFileContent(
                virtualPath, 
                Number(byteOffset), 
                content
            );
     ...
     * Handle file deletion
     */
    private async handleFileDeletion(virtualPath: string): Promise<void> {
