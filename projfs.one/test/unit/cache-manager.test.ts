...
import type { PlaceholderInfo, DirectoryEntry } from '../../src/native/projfs-types.js';

describe('CacheManager', () => {
    let cache: CacheManager;
    
    beforeEach(() => {
        cache = new CacheManager(1024 * 1024); // 1MB
    });
    ...
    describe('File Info Cache', () => {
        it('should store and retrieve file info', async () => {
            ...
            expect(retrieved).to.deep.equal(info);
        });
        ...
        it('should update existing file info', async () => {
            ...
            const retrieved = await cache.getFileInfo('/test.txt');
            expect(retrieved?.fileSize).to.equal(2048n);
        });
    });
    ...
    describe('Content Cache', () => {
        it('should store and retrieve content chunks', async () => {
            ...
            expect(retrieved).to.deep.equal(chunk);
        });
        ...
        it('should handle partial chunk retrieval', async () => {
            ...
            expect(retrieved).to.deep.equal(Buffer.from([3, 4, 5]));
        });
        ...
        it('should merge adjacent chunks', async () => {
            ...
            expect(retrieved).to.deep.equal(Buffer.from([1, 2, 3, 4, 5, 6, 7, 8]));
        });
    ...
    describe('Path Invalidation', () => {
        ...
        it('should invalidate all children of a path', async () => {
            await cache.putFileInfo('/folder/file1.txt', {} as PlaceholderInfo);
            await cache.putFileInfo('/folder/file2.txt', {} as PlaceholderInfo);
            await cache.putFileInfo('/folder/subfolder/file3.txt', {} as PlaceholderInfo);
            await cache.putFileInfo('/other/file.txt', {} as PlaceholderInfo);
            
            await cache.invalidatePath('/folder');
            
            expect(await cache.getFileInfo('/folder/file1.txt')).to.be.undefined;
            expect(await cache.getFileInfo('/folder/file2.txt')).to.be.undefined;
            expect(await cache.getFileInfo('/folder/subfolder/file3.txt')).to.be.undefined;
            expect(await cache.getFileInfo('/other/file.txt')).to.not.be.undefined;
        });
    });
    ...
    describe('Cache Statistics', () => {
        it('should track cache statistics', () => {
            ...
            expect(stats).to.have.property('metadataEntries');
            expect(stats).to.have.property('directoryEntries');
            expect(stats).to.have.property('contentEntries');
            expect(stats).to.have.property('metadataSize');
            expect(stats).to.have.property('directorySize');
            expect(stats).to.have.property('contentSize');
        });
        
        it('should clear cache when requested', async () => {
            await cache.putFileInfo('/test.txt', {} as PlaceholderInfo);
            ...
            const stats = cache.getStats();
            expect(stats.metadataEntries).to.equal(0);
            expect(stats.directoryEntries).to.equal(0);
            expect(stats.contentEntries).to.equal(0);
        });
    });
});